...
```ts
  ...
  14 | 
> 15 |     const calender = await page.locator('#profile-date').fill('09-01-1988');
     |                                                          ^ Error: locator.fill: Error: Malformed value
  16 | 
  17 |     const profession = await page.getByTestId('profession-automation').click();
  18 | 
  19 |     const technicalskil = await page.getByTestId('tool-selenium').isChecked();
  20 | 
  21 |     const continent = await page.getByTestId('continent-{name: "Asia"}').isChecked();
  22 | 
  23 |     const uploadimg = await page.getByTestId('upload-image').setInputFiles('test-data/Screenshot (147).png');
  24 | 
  25 | 
  26 | 
  27 | 
